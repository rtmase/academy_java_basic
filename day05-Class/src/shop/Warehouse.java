...
/**
 * 매장에 판매되는 제품들을 모아두는 창고를 구현하는 클래스
 * 제품 정보를 저장하기 위하여 배열로 관리
 * 
 * 제품들이 올려져있거나 쌓여있는 창고의 선반은 
 * 각 창고마다 존재하기 때문에 구현은 멤버변수(실제변수)로 선언해야 함.
 * 
 * 메소드
...
public class Warehouse {
	// 멤버 변수 선언 : 제품을 쌓아둘 배열 참조 변수
	Product[] products;
	
	// 생성자 선언
	/**
	 * 기본 생성자 : 창고를 처음 지을 대 어떻게 지을 것인지 방법을 알려주는 생성자
	 */
	Warehouse() {
		// 옵션 없이 창고를 지으면 선발 길이를 0으로 만든다.
	...
	*/
	...
	/**
	 * 파라미터로 전달된 정보와 일치하는 제품이 있는지 검색하여( prodCode 필드가 같으면 같은 제품으로 처리)
	 * 일치하는 제품이 있으면 그 제품정보를 리턴 없으면 null 을 리턴
	 * @param product
	 * @return
	 */
	...
	/**
	 * 매개변수로 넘겨진 제품 정보와 일치하는 제품코드를 가진 제품에 대해
	 * 입력된 값으로 수정하여 저장함.
	 * @param product
	 * @return
	 */
	public void set(Product product) {
		// 수정하고자 하는 제품의 인덱스
		int setIndex = -1;
		setIndex = findProductIdx(product);
	...
	private Product findProduct(Product product) {
		...
		for(Product prod : products) {
			// 배열에 존재하는 제품이 정보(prod)가 매개변수로 넘겨진 제품 정보(product)와 일치하는 비교 : prodCode만 비교
	...
	private int findProductIdx(Product product) {
		...
		for (int idx = 0; idx < products.length; idx++) {
			// 배열의 인덱스에서 추출한 제품 정보 한개 products[idx]의 제품 코드 필드(prodCode) 가 
			// 매개변수로 넘어온 product 의 제품 코드 필드와 동일한지 빅하고 그 때의 배열 인덱스를 저장
